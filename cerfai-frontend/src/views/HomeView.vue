<template>
  <div class="home">
    <el-menu :default-active="activeIndex" class="menu" mode="horizontal" @select="handleSelect">
      ...
      <el-menu-item index="tag">发起贡献 ></el-menu-item>
    ...
    <div class="container" v-show="activeIndex == 'home'">
      <!--入口页，介绍本站，提供一个前往贡献页面的入口-->
      <div class="common-layout">
        <el-container>
          <el-header><h1 :style="`font-size: var(--el-font-size-extra-large)`">Novelai词条贡献站</h1></el-header>
          <el-main>
            <h2 :style="`font-size: var(--el-font-size-large)`">简介</h2><br />
            ...
            本站交流群：660612010<br />
            目前已收到用户贡献词条数: {{ up_cnt }}<br />
            当前站点已收录总词条数: {{ ct_total }}<br />
            <el-divider />
            <h2 :style="`font-size: var(--el-font-size-large)`">参与贡献</h2><br />
            点击前往词条解释页面，为词条解释数据库做出贡献
            <el-button type="primary" round @click="activeIndex = 'tag'">前往编辑词条数据库→</el-button>
            <el-divider />
            <h2 :style="`font-size: var(--el-font-size-large)`">参与贡献基本原则</h2><br />
            在不破坏他人劳动成果的前提下，对词条原有信息做任何有效补充均可视为有效贡献<br />
            您所看到的所有词条内容来自 机器翻译/人工初筛/其他用户个人上传。<br />
            欢迎对词条进行内容审核或者参与实际测试，并提供宝贵的建议。<br />
            如果您认为该词条准确无误，请点击提交，将自动随机下一条标签。<br />
            输入词条分类关键词，可以快速选择词条分类。<br />
            如果该词条没有适合的分类，请输入并选择 “标签扩展” 并可在备注中描述新的分类，<br />
            如果经多方测试，标签无效或者当前版本无效，请输入并选择“无效标签”<br />
            如果您对当前标签没兴趣或者不需要进行修改，点击“换一个”更换词条。<br />
            <strong>词条名：</strong><br />
            是指导入数据库的Tag英文原名，软件可识别的词条/标签。<br />
            <strong>词条译名：</strong><br />
            是指词条翻译的中文名，或者贡献者修改的译名，最好是中文版AI可识别的译名。因为尚未校正，可能存在翻译错误。<br />
            <strong>是否NSWF：</strong><br />
            当前标签生成图片时是否会出现不适合工作场合的画面，包括不限于血腥暴力和敏感内容，如果是请打开此项。<br />
            <strong>词条说明：</strong><br />
            对于词条的补充和内容说明。对标签进行测试时，建议标明SD测试还是NF测试，两者会有些许差异。单独使用标签与实际混合标签时会产生不同效果，cfg参数会影响标签效果，标签数字权重与{}的增加也会标签产生的效果，建议多次测试确定效果。<br />
            <strong>备注信息：</strong><br />
            词条的来源和调查。词条的正确译名可能需要借助谷歌等搜索引擎，容易产生误解的是画师账号昵称和人名，并且可能由于关联搜索而出现奇怪的内容，导致内容与标签毫无关联，这取决于模型的收录程度，如果要强调画师或者风格，应该加上by_或者（style）<br />
            <strong>贡献者：</strong><br />
            所有参与词条编辑的人都可以填入贡献者名字，请使用常用的昵称或者数字字母，提交词条将计入贡献名单，优秀的内容提交者将在榜单上体现，请尊重自己和他人的劳动成果，恶意删改将会触发网站防御，并拉入AI绘画信息并联计划黑名单。<br />
          </el-main>
        </el-container>
      </div>
  </div>
  <div class="container" v-show="activeIndex == 'tag'">
    <!--该分页防止贡献提交表单-->
    <!-- 贡献表单 -->
    <el-form ref="form" :model="form" label-width="100px">
      ...
      <el-form-item label="词条分类：">
        <el-autocomplete class="inline-input" v-model="form.c_name" :fetch-suggestions="querySearch"
          placeholder="可搜索词条分类" @select="categoryHandleSelect" clearable></el-autocomplete>
      </el-form-item>
  ...
  <div class="container">
    ...
    <div class="search" v-show="activeIndex == 'tag_search'">
      ...
      <el-table @row-click="getRow" :data="search_res" class="searchTable" border fit style="width: 100%">
        ...
        <el-table-column prop="c_id" label="分类id"></el-table-column>
    ...
    <div class="share" v-show="activeIndex == 'share'">
      <p>NovelAI词条贡献站 - 数据开放平台 根据 <a href="https://www.bilibili.com/read/cv19252957" target="_blank">NovelAI信息并联计划公约</a>
        约定：</p>
      <p>
        贡献站所有词条数据资源来源于用户/网络搜集，免费服务于所有用户，任何个人/团队均可免费使用本站资源于任何 <strong style="color:#e55;">非商用/盈利目的</strong> 项目<br />
  ...
  methods: {
    ...
    toDonate() {
      ...
      setTimeout(() => {
        ...
        _this.form.is_nfsw = _this.selRowData.is_nfsw
